...
- **Exact phrases: "restore", "restore session", or "restore the stack"** mean run `/home/glilly/ops/scripts/cursor-session-stack.sh restore` (permissions that allow local listeners/Docker). Always brings up both UI gateways: vehu10 on `5177` → `9085`, and `rpms-rebuild-candidate` on `5178` → `9088`. Snapshot file: `~/.cursor/vista-session-stack.json`. Use **"snapshot"** / **"remember the stack"** to update that file after starting demos. Cursor `sessionEnd` also auto-snapshots when hooks are enabled.
