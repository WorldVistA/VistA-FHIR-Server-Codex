...
- **Exact phrases: "restore", "restore session", or "restore the stack"** mean run `/home/glilly/ops/scripts/cursor-session-stack.sh restore` (permissions that allow local listeners/Docker). This restarts whatever was last snapped in `~/.cursor/vista-session-stack.json` (default: vehu10 FHIR UI gateway on `5177`). Use **"snapshot"** / **"remember the stack"** to update that file after starting demos. Cursor `sessionEnd` also auto-snapshots when hooks are enabled.
