...
- **Exact phrase: "start the FHIR UI gateway server to vehu10"** means the user demos from the backend-generated FHIR patient index at `http://localhost:5177/fhir`, then clicks the `rehmp` link in that table to open the CPRS demo. Start both pieces when needed:
  - Working directory: `/home/glilly/work/vista-stack/rehmp/ehmp-ui/rehmp-fhir-demo`
  - Command: `npm run dev:wsl:links:vehu10` (serves `/fhir` on port `5177` and proxies backend paths to `vehu10`)
  - If `/demos/cprs/...` through port `5177` returns `500`, also start the CPRS UI target:
    - Working directory: `/home/glilly/work/vista-stack/rehmp/ehmp-ui/rehmp-cprs-demo`
    - Command: `npm run dev:wsl:vehu10 -- --port 5175`
  ...
  - Do not treat `http://localhost:5177/demos/fhir/` as the demo entry point for this request; `/fhir` is the entry point.
