...
BROWSER(RTN,DFN) ; React-style interactive FHIR browser
 ...
 D ADDLN(.RTN,".wrap{display:flex;height:calc(100vh - 64px);}")
 D ADDLN(.RTN,".left{width:360px;min-width:300px;border-right:1px solid #334155;display:flex;flex-direction:column;}")
 D ADDLN(.RTN,".right{flex:1;display:flex;flex-direction:column;}")
 ...
 D ADDLN(.RTN,".list{overflow:auto;flex:1;}")
 ...
 D ADDLN(.RTN,"const st={all:[],rows:[],pick:null,q:'',type:'all'};")
 ...
 D ADDLN(.RTN," if(!st.rows.length){st.pick=null;return;}")
 D ADDLN(.RTN," if(st.pick&&st.rows.indexOf(st.pick)>-1) return;")
 D ADDLN(.RTN," st.pick=st.rows[0];}")
 D ADDLN(.RTN,"function drawList(){const n=el('list');let h='';")
 D ADDLN(.RTN," st.rows.forEach((e,i)=>{const a=(st.pick===e)?' active':'';")
 D ADDLN(.RTN," h+=""<div class='item""+a+""' data-i='""+i+""'><div class='rt'>""+esc(rtype(e))+""</div><div class='nm'>""+esc(titleOf(e))+""</div><div class='id'>id: ""+esc(rid(e))+""</div></div>"";});")
 D ADDLN(.RTN," if(!h) h=""<div class='item'>No resources match.</div>"";")
 D ADDLN(.RTN," n.innerHTML=h;")
 D ADDLN(.RTN," n.querySelectorAll('.item[data-i]').forEach(x=>x.onclick=()=>{st.pick=st.rows[+x.dataset.i];draw();});}")
 D ADDLN(.RTN,"function drawDetail(){")
 D ADDLN(.RTN," el('meta').textContent=st.rows.length+' of '+st.all.length+' resources';")
