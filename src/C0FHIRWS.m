...
BROWSER(RTN,FILTER) ; Interactive FHIR browser for live /fhir or stored /showfhir bundles
 ...
 D ADDLN(.RTN,".quality{margin:10px 16px 0;padding:10px;border:1px solid var(--divider);border-radius:8px;background:var(--panel);}")
 D ADDLN(.RTN,".quality h3{margin:0 0 8px;font-size:15px}.quality-controls{display:flex;gap:8px;align-items:center;flex-wrap:wrap}")
 D ADDLN(.RTN,".quality input{width:260px;max-width:100%}.quality button{padding:7px 10px;border:1px solid var(--btn-border);border-radius:6px;background:var(--btn-bg);color:var(--fg);cursor:pointer}")
 D ADDLN(.RTN,".quality button:disabled{opacity:.5;cursor:not-allowed}.quality-status{font-size:12px;color:var(--muted);margin-top:6px}")
 D ADDLN(.RTN,".quality-actions{margin-top:8px;display:grid;gap:6px}.quality-action{border:1px solid var(--item-border);border-radius:6px;padding:7px}")
 D ADDLN(.RTN,".quality-action label{display:flex;gap:8px;align-items:flex-start}.quality-action input{width:auto;margin-top:2px}.quality-action small{color:var(--muted)}")
 ...
 I D>0 D
 . D ADDLN(.RTN,"<div id='qualityPanel' class='quality'>")
 . D ADDLN(.RTN,"<h3>Quality AI Consult</h3>")
 . D ADDLN(.RTN,"<div class='quality-controls'><input id='qualityMeasures' value='CMS165v14,CMS122v14' aria-label='Quality measures'>")
 . D ADDLN(.RTN,"<button type='button' id='qualityAnalyze'>Analyze</button><button type='button' id='qualitySave' disabled>Save Review</button><button type='button' id='qualityApply' disabled>Apply Review</button></div>")
 . D ADDLN(.RTN,"<div id='qualityStatus' class='quality-status'>Loading quality pick-list actions...</div>")
 . D ADDLN(.RTN,"<div id='qualityActions' class='quality-actions'></div></div>")
 ...
 D ADDLN(.RTN,"function extVal(parts,name){")
 D ADDLN(.RTN," const x=(parts||[]).find(p=>p&&p.url===name)||{};")
 D ADDLN(.RTN," return x.valueString||x.valueCode||x.valueBoolean||'';}")
 D ADDLN(.RTN,"function qualityActionsFrom(bundle){const out=[];")
 D ADDLN(.RTN," ((bundle&&bundle.entry)||[]).forEach(e=>{const r=(e||{}).resource||{};")
 D ADDLN(.RTN,"  if(r.resourceType!=='DiagnosticReport') return;")
 D ADDLN(.RTN,"  (r.extension||[]).forEach(ex=>{")
 D ADDLN(.RTN,"   if(!ex||ex.url!=='https://github.com/glilly/cds-hooks-on-fhir/StructureDefinition/quality-pick-list-action') return;")
 D ADDLN(.RTN,"   const parts=ex.extension||[];")
 D ADDLN(.RTN,"   const id=extVal(parts,'actionId');if(!id) return;")
 D ADDLN(.RTN,"   out.push({id,measure:extVal(parts,'measureId'),role:extVal(parts,'cohortRole'),")
 D ADDLN(.RTN,"    target:extVal(parts,'targetResourceType'),display:extVal(parts,'display'),")
 D ADDLN(.RTN,"    support:extVal(parts,'supportText')});")
 D ADDLN(.RTN,"  });")
 D ADDLN(.RTN," });return out;}")
 D ADDLN(.RTN,"function drawQualityActions(actions){const box=el('qualityActions'),save=el('qualitySave');")
 D ADDLN(.RTN," if(!box||!save) return;")
 D ADDLN(.RTN," box.innerHTML='';")
 D ADDLN(.RTN," if(!actions.length){save.disabled=true;return;}")
 D ADDLN(.RTN," actions.forEach(a=>{")
 D ADDLN(.RTN,"  const div=document.createElement('div');div.className='quality-action';")
 D ADDLN(.RTN,"  const meta=[a.measure,a.role,a.target].filter(Boolean).join(' / ');")
 D ADDLN(.RTN,"  div.innerHTML=""<label><input type='checkbox' class='qa'><span><strong></strong> <small></small><br><small></small></span></label>"";")
 D ADDLN(.RTN,"  div.querySelector('input').value=a.id;")
 D ADDLN(.RTN,"  div.querySelector('strong').textContent=a.display||a.id;")
 D ADDLN(.RTN,"  div.querySelectorAll('small')[0].textContent=meta;")
 D ADDLN(.RTN,"  div.querySelectorAll('small')[1].textContent=a.support||'';")
 D ADDLN(.RTN,"  box.appendChild(div);")
 D ADDLN(.RTN," });save.disabled=false;}")
 D ADDLN(.RTN,"async function analyzeQuality(){const panel=el('qualityPanel');if(!panel||!dfn)return;")
 D ADDLN(.RTN," const status=el('qualityStatus');const measures=el('qualityMeasures').value||'CMS165v14,CMS122v14';")
 D ADDLN(.RTN," status.textContent='Analyzing quality gaps...';el('qualityActions').innerHTML='';el('qualitySave').disabled=true;")
 D ADDLN(.RTN," if(el('qualityApply'))el('qualityApply').disabled=true;st.qualityReviewId='';")
 D ADDLN(.RTN," const url='/aiconsult?dfn='+dfn+'&format=json&file=0&mode=quality&measure='+encodeURIComponent(measures);")
 D ADDLN(.RTN," const r=await fetch(url);if(!r.ok)throw new Error('HTTP '+r.status+' from quality analysis');")
 D ADDLN(.RTN," const b=await r.json();const actions=qualityActionsFrom(b);st.qualityActions=actions;drawQualityActions(actions);")
 D ADDLN(.RTN," status.textContent=actions.length?('Found '+actions.length+' review action(s).'):'No quality pick-list actions found.';}")
 D ADDLN(.RTN,"async function saveQualityReview(){const status=el('qualityStatus');")
 D ADDLN(.RTN," const ids=Array.from(document.querySelectorAll('.qa:checked')).map(x=>x.value);")
 D ADDLN(.RTN," if(!ids.length){status.textContent='Select at least one action to save.';return;}")
 D ADDLN(.RTN," status.textContent='Saving review artifact...';")
 D ADDLN(.RTN," const r=await fetch('/aiconsult/update-review?dfn='+dfn,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({acceptedActions:ids})});")
 D ADDLN(.RTN," if(!r.ok)throw new Error('HTTP '+r.status+' saving review');")
 D ADDLN(.RTN," const j=await r.json();")
 D ADDLN(.RTN," st.qualityReviewId=j.id;if(el('qualityApply'))el('qualityApply').disabled=false;")
 D ADDLN(.RTN," status.innerHTML=`Saved review <a href=""/writebacksaves/${esc(j.id)}"">${esc(j.id)}</a>`;}")
 D ADDLN(.RTN,"async function applyQualityReview(){const status=el('qualityStatus');")
 D ADDLN(.RTN," const id=st.qualityReviewId;if(!id){status.textContent='Save a review before applying.';return;}")
 D ADDLN(.RTN," status.textContent='Applying quality review through updatepatient...';")
 D ADDLN(.RTN," const r=await fetch('/aiconsult/apply-review?id='+encodeURIComponent(id),{method:'POST',headers:{'content-type':'application/json'},body:'{}'});")
 D ADDLN(.RTN," if(!r.ok)throw new Error('HTTP '+r.status+' applying review');")
 D ADDLN(.RTN," const j=await r.json();")
 D ADDLN(.RTN," status.innerHTML=`Applied quality review from <a href=""/writebacksaves/${esc(id)}"">${esc(id)}</a>`;}")
 ...
 D ADDLN(.RTN,"if(el('qualityAnalyze')) el('qualityAnalyze').addEventListener('click',()=>analyzeQuality().catch(e=>{el('qualityStatus').textContent=String(e);}));")
 D ADDLN(.RTN,"if(el('qualitySave')) el('qualitySave').addEventListener('click',()=>saveQualityReview().catch(e=>{el('qualityStatus').textContent=String(e);}));")
 D ADDLN(.RTN,"if(el('qualityApply')) el('qualityApply').addEventListener('click',()=>applyQualityReview().catch(e=>{el('qualityStatus').textContent=String(e);}));")
 D ADDLN(.RTN,"if(el('qualityPanel')) analyzeQuality().catch(e=>{el('qualityStatus').textContent=String(e);});")
