...
RPMS() ; $$ - true when RPMS PCC visit filing is available
 ...
 Q 1
 ;
RPMSLOAD(ROOT,IEN,RIEN,DFN,FMDT,LOC,USER,KNOWNVISIT,RETURN) ; File RPMS outpatient visit via authorized PCC APIs
 N APDT,ERR,VISIT
 S APDT=$$APPTDT(FMDT)
...
RPMSEXI(DFN,LOC,APDT) ; $$ - existing visit for patient/date/location
 ...
 Q +VISIT
 ;
RPMSVIS(ERR,DFN,LOC,APDT) ; $$ - create/reuse visit through authorized RPMS PCC API
 N APCDALVR,APCDAUTO,APCDCAT,APCDCLN,APCDDATE,APCDHL,APCDLOC,APCDPAT,APCDTYPE,APCDVSIT,FAC
 S ERR=""
 S FAC=$$FAC(LOC)
 I FAC<1 S ERR="Unable to resolve RPMS facility for visit" Q 0
 K APCDALVR
 S APCDPAT=DFN
 S APCDDATE=APDT
 S APCDTYPE="I"
 S APCDLOC=FAC
 S APCDCAT=$$SERCAT(APDT)
 S APCDHL=LOC
 S APCDCLN=$$RPMSCLIN(LOC)
 S APCDAUTO=""
 D EN^APCDALV
 I $D(APCDAFLG) S ERR="APCDALV failed to create RPMS visit"_$S($G(APCDAFLG("ERR"))'="":": "_$G(APCDAFLG("ERR")),1:"") Q 0
 I +$G(APCDVSIT)<1 S ERR="APCDALV did not return an RPMS visit IEN" Q 0
 Q +APCDVSIT
 ;
FAC(LOC) ; $$ - RPMS facility for a hospital location
 N FAC
 S FAC=+$P($G(^SC(+$G(LOC),0)),U,4)
 I FAC>0,$D(^AUTTLOC(FAC,0)) Q FAC
 S FAC=+$G(DUZ(2))
 I FAC>0,$D(^AUTTLOC(FAC,0)) Q FAC
 Q +$O(^AUTTLOC(0))
 ;
RPMSSTAT(ROOT,IEN,RIEN,VISIT,APDT,LOC) ; Persist RPMS filing detail
...
RPMSHF(ERR,DFN,VISIT,FMDT,LOC,USER,ENCDATA,ROOT,IEN,RIEN) ; File queued RPMS V HEALTH FACTOR rows
 N CLIN,CMT,HF,HFIEN,NEWIEN,SEV,VARS
 S ERR=""
 ...
 . S CLIN=$$RPMSCLIN(LOC)
 . K VARS
 . S VARS("APCDTHF")="`"_HFIEN
 . I SEV'="" S VARS("APCDTLS")=SEV
 . S VARS("APCDTCDT")=FMDT
 . I CLIN>0 S VARS("APCDTCLN")=CLIN
 . S VARS("APCDTEPR")=USER
 . I CMT'="" S VARS("APCDTCOM")=$E(CMT,1,245)
 . S NEWIEN=$$RPMSVF(.ERR,DFN,VISIT,"[APCDALVR 9000010.23 (ADD)]",.VARS)
 . I $G(ERR)'="" Q
 . D RPHFST(ROOT,IEN,RIEN,HFIEN,"filed","Health Factor filed as RPMS V HEALTH FACTOR: "_$P($G(^AUTTHF(HFIEN,0)),U))
 Q
 ;
RPMSVF(ERR,DFN,VISIT,TEMPLATE,VARS) ; $$ - file one RPMS V-file entry through APCDALVR
 N APCDALVR,NM
 S ERR=""
 I +$G(VISIT)<1 S ERR="Missing RPMS visit for V-file filing" Q 0
 I +$G(DFN)<1 S ERR="Missing RPMS patient for V-file filing" Q 0
 K APCDALVR
 S APCDALVR("APCDPAT")=+DFN
 S APCDALVR("APCDVSIT")=+VISIT
 S APCDALVR("APCDATMP")=$G(TEMPLATE)
 S APCDALVR("APCDAFLE")=9000010
 S NM="" F  S NM=$O(VARS(NM)) Q:NM=""  S APCDALVR(NM)=VARS(NM)
 D EN^APCDALVR
 I $G(APCDALVR("APCDAFLG")) S ERR="APCDALVR failed for "_$G(TEMPLATE)_": "_$G(APCDALVR("APCDAFLG")) Q 0
 Q +$G(APCDALVR("APCDADFN"))
 ;
RPMSMSR(ERR,DFN,VISIT,MSR,VAL,FMDT,LOC,USER) ; $$ - file V MEASUREMENT through APCDALVR
 N CLIN,VARS
 S CLIN=$$RPMSCLIN(LOC)
 K VARS
 S VARS("APCDTTYP")="`"_+$G(MSR)
 S VARS("APCDTVAL")=$G(VAL)
 S VARS("APCDTCDT")=$G(FMDT)
 I CLIN>0 S VARS("APCDTCLN")=CLIN
 S VARS("APCDTEPR")=+$G(USER)
 Q $$RPMSVF(.ERR,DFN,VISIT,"[APCDALVR 9000010.01 (ADD)]",.VARS)
 ;
RPMSIMM(ERR,DFN,VISIT,IMMIEN,FMDT,LOC,USER,SERIES) ; $$ - file V IMMUNIZATION through APCDALVR
 N CLIN,VARS
 S CLIN=$$RPMSCLIN(LOC)
 K VARS
 S VARS("APCDTIMM")="`"_+$G(IMMIEN)
 I $G(SERIES)'="" S VARS("APCDTSER")=$G(SERIES)
 S VARS("APCDTCDT")=$G(FMDT)
 I CLIN>0 S VARS("APCDTCLN")=CLIN
 S VARS("APCDTEPR")=+$G(USER)
 Q $$RPMSVF(.ERR,DFN,VISIT,"[APCDALVR 9000010.11 (ADD)]",.VARS)
 ;
RPMSPOV(ERR,DFN,VISIT,ICD,NARR,FMDT,LOC,USER,PRIMARY,SCT) ; $$ - file V POV through APCDALVR
 N CLIN,VARS
 S CLIN=$$RPMSCLIN(LOC)
 K VARS
 S VARS("APCDTPOV")="`"_+$G(ICD)
 S VARS("APCDTNQ")=$S($G(NARR)?1N.N:"`"_+$G(NARR),1:$G(NARR))
 S VARS("APCDTPS")=$S(+$G(PRIMARY):"P",1:"S")
 S VARS("APCDTCDT")=$G(FMDT)
 I CLIN>0 S VARS("APCDTCLN")=CLIN
 S VARS("APCDTEPR")=+$G(USER)
 I $G(SCT)'="" S VARS("APCDTSNO")=$G(SCT)
 Q $$RPMSVF(.ERR,DFN,VISIT,"[APCDALVR 9000010.07 (ADD)]",.VARS)
 ;
