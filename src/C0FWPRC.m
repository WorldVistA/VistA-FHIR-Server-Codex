...
LOADVEHU(ROOT,IEN,RIEN,RETURN) ; VEHU/SYN path via PRCADD^SYNDHP65
 N CNT,CPTIEN,DFN,FMDT,HL7DT,ICN,MSG,OS5,RETSTA,SCT,TYPE,VISIT,X
 S TYPE="Procedure"
 S X="PRCADD^SYNDHP65"
 ...
 . D LOG(ROOT,IEN,RIEN,$G(OS5),"SYN/PCE","PRCADD^SYNDHP65",VISIT,SCT,$G(CPTIEN),HL7DT,"loaded",$G(RETSTA))
 I $G(CPTIEN)>0,$$HASCPT(VISIT,CPTIEN) D  Q
 . S MSG="Procedure already matched visit-linked V CPT after PRCADD: SCT="_SCT
 . D SET^C0FWSTAT(ROOT,IEN,RIEN,"Procedure",TYPE,"loaded",MSG,.RETURN)
 . S RETURN("domains","Procedure","visitIen")=VISIT
 . D LOG(ROOT,IEN,RIEN,OS5,"SYN/PCE","PRCADD^SYNDHP65",VISIT,SCT,CPTIEN,HL7DT,"loaded",$G(RETSTA))
 I $G(CPTIEN)>0 D  I $$OKRPMS($G(RETSTA),VISIT,CPTIEN) Q
 . S FMDT=$$FMDT(ROOT,IEN,RIEN,VISIT)
 . I FMDT<1 Q
 . S RETSTA=$$FILEPCE(VISIT,CPTIEN,FMDT)
 . I '$$OKRPMS(RETSTA,VISIT,CPTIEN) Q
 . S MSG="Procedure filed through VEHU FILEPCE fallback: CPT="_OS5_" SCT="_SCT
 . D SET^C0FWSTAT(ROOT,IEN,RIEN,"Procedure",TYPE,"loaded",MSG,.RETURN)
 . S RETURN("domains","Procedure","visitIen")=VISIT
 . D LOG(ROOT,IEN,RIEN,OS5,"VEHU/PCE","FILEPCE^C0FWPRC",VISIT,SCT,CPTIEN,HL7DT,"loaded",$G(RETSTA))
 S MSG=$G(RETSTA)
 I MSG="" S MSG="-1^PRCADD^SYNDHP65 returned empty status"
 D ERR^C0FWSTAT(ROOT,IEN,RIEN,"Procedure",TYPE,"PRCADD failed: "_MSG,.RETURN)
 D LOG(ROOT,IEN,RIEN,$G(OS5),"SYN/PCE","PRCADD^SYNDHP65",VISIT,SCT,$G(CPTIEN),HL7DT,"error",MSG)
 Q
 ;
