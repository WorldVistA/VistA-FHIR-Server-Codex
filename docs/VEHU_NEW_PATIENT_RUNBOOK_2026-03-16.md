...
- **`fhirdev-push-synfhir.sh`** — `scp` + `docker cp` **`SYNFHIR.m`** into the fhirdev container routine dir (default **`/home/osehra/p`**; set **`FHIRDEV_ROUTINE_DIR`** if your image uses **`/home/vehu/p`**). After copy, reload in M and **`XINDEX SYNFHIR`**. **`VistA-FHIR-Data-Loader` `master`** already carries **`wsShow^SYNFHIR`**, and the **`fhir`** / **`vehu9`** images match that; **`fhirdev22`** is often built from an integration branch (e.g. **`vaready-wd-compat`**) whose **`SYNFHIR`** may omit **`wsShow`** until **`master`** is merged or **`SYNFHIR.m`** is copied from **`fhir`** / **`master`**. If **`showfhir`** returns **`LABELMISSING wsShow`**, align **`SYNFHIR`** with **`master`** (or another container that lists **`wsShow`** on **`/r/SYNFHIR`**).
