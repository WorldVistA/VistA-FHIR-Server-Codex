...
## 2026-04-11
- Investigated `vendev15.vistaplex.org` / Docker container `vapals` after reports of severe slowdown and confirmed **121** runaway `/usr/local/lib/yottadb/r122/mumps -run GTMLNX^HLCSGTM` jobs. Stopped them from inside the container as `osehra` using `/home/osehra/etc/env.conf` and `mupip stop <pid>`, then rechecked that the listener count stayed at `0` and host load fell from the high-60s to near idle.
- Confirmed `vapals` exposed the same unsafe SSH pattern seen on the other reviewed servers: host `2222 -> container 22`, `sshd` running in the container, `PermitRootLogin yes`, `PasswordAuthentication yes`, and a valid container `root` password hash. Locked container `root`, changed `sshd_config` to disable root/password auth, stopped container `sshd`, and blocked host `2222/tcp` with both `ufw` and `iptables` while preserving the FHIR demo listener on `9080`.
- Diagnosed the apparent `su - osehra` hang inside `vapals` as a login-accounting problem rather than an M or shell environment failure. `strace` and PAM review showed `pam_lastlog` scanning a **13 GiB** `/var/log/btmp` file created by repeated failed logins against the exposed SSH path. Truncated `/var/log/btmp` to `0 bytes`, after which both `su - osehra -c date` and `su osehra -c true` returned immediately again.
- Verified that the known `/.mod` / `/usr/lib/libgdi.so.0.8.2` malware chain was **not** present in `vapals`, `/etc/crontab` was clean of the old launcher entry, and documented the full `vendev15` findings and containment in `docs/VENDEV15_INCIDENT_RESPONSE_2026-04-11.md`.
- Investigated `postkit3.vistaplex.org` / Docker container `postkit` and found the same broad SSH foothold pattern as the other servers: host `2222 -> container 22`, `sshd` and `xinetd` running in the container, `PermitRootLogin yes`, a valid container `root` password hash, and the suspicious root authorized key with fingerprint `SHA256:MkYY9qiVsFGBC5WkjoClCkwEFW5iSjcGQF7m4n4H7Cw` / comment `mdrfckr`. No `/.mod` malware chain was present and no runaway `GTMLNX^HLCSGTM` swarm was found.
- On `postkit3`, identified a separate `su - osehra` delay caused by `pam_lastlog` scanning a **2.6 GiB** `/var/log/btmp` file, with additional login-shell drag from `source $HOME/.nvm/nvm.sh` and `nvm use 6` in `/home/osehra/.bash_profile`. Cleared the suspicious container root key, locked container `root`, disabled root/password SSH in the container, stopped `sshd` / `xinetd`, truncated `/var/log/btmp`, disabled `nvm use 6` in `osehra`'s login profile, and verified both `su - osehra -c date` and `runuser -l osehra -c date` returned immediately again.
- Removed host `ufw` allow rules for `2375/tcp` and `2376/tcp` on `fhirdev.vistaplex.org`, `fhir.vistaplex.org`, `vendev15.vistaplex.org`, and `postkit3.vistaplex.org`, then rechecked that none of those hosts still had `2375` / `2376` rules present. On `postkit3`, also added `ufw deny 2222/tcp`, `ufw route deny 2222/tcp`, and a direct `iptables` drop for `2222/tcp`; external probing then showed host `22` still reachable while `2222` returned `Connection refused`.
- Documented the full `postkit3` findings and containment in `docs/POSTKIT3_INCIDENT_RESPONSE_2026-04-11.md`.
- Reviewed the three local Docker containers (`fhir`, `vehu10`, `vehu6`) using the same SSH/malware/login-delay checklist as the remote servers. Found no `/.mod` malware chain locally, but all three still had active container SSH exposed through published host `->22` mappings. The `fhir` container also had the suspicious `mdrfckr` root key, a set root password, a **1.5 GiB** `/var/log/btmp`, and `osehra` login-shell NVM auto-use causing `su - osehra` delay.
- Hardened the local containers in place: cleared root `authorized_keys`, disabled root/password SSH in `sshd_config`, stopped `sshd` / `xinetd` in all three, truncated `/var/log/btmp` in `fhir`, and disabled `nvm use 6` in `/home/osehra/.bash_profile` in `fhir`. Verified `su - osehra` / `runuser -l osehra` worked immediately again in `fhir`, `su - vehu` / `runuser -l vehu` worked in `vehu10` and `vehu6`, and that `fhir` / `vehu10` still returned HTTP `200` on their local `/fhir` paths while `vehu6` still returned `000`.
- Documented the full local-container findings, containment steps, and the explicit operator decision to leave the laptop's published Docker `->22` mappings in place in `docs/LOCAL_CONTAINERS_INCIDENT_RESPONSE_2026-04-11.md`.
