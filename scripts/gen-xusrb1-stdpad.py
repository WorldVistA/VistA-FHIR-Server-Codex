...
# sanity: each pad must be 94 chars and a permutation of printable(32..126) minus '^'
allowed = set(chr(c) for c in range(32,127)) - {'^'}
for i,p in enumerate(client):
    assert len(p)==94, (i,len(p))
    assert set(p)==allowed, (i, "charset mismatch", sorted(set(p)^allowed))
    assert p.count(';')==1, (i, "semicolons", p.count(';'))
print("all 20 client pads valid: 94 chars, permutation, single ';'")

head = [
"ROUTINE XUSRB1 [Type=MAC]",
...
lines = list(head) + [" ;;"+p for p in client]
with open("/tmp/XUSRB1.mac","w") as f:
    f.write("\n".join(lines)+"\n")
print("wrote /tmp/XUSRB1.mac with", len(lines), "lines")
