...
need=(C0FHIRLG.m C0FHIRL.m C0FWLAB.m C0FWPOL.m)
for f in "${need[@]}"; do
...
"${SCP[@]}" \
  "$ROOT/src/C0FHIRLG.m" "$ROOT/src/C0FHIRL.m" \
  "$ROOT/src/C0FWLAB.m" "$ROOT/src/C0FWPOL.m" \
  "$SSH_HOST:$STAGE/"

echo "==> docker cp + chown into $CONT:$REMOTE_P"
"${SSH[@]}" "$SSH_HOST" "docker cp '$STAGE/C0FHIRLG.m' '$CONT:$REMOTE_P/C0FHIRLG.m' && docker cp '$STAGE/C0FHIRL.m' '$CONT:$REMOTE_P/C0FHIRL.m' && docker cp '$STAGE/C0FWLAB.m' '$CONT:$REMOTE_P/C0FWLAB.m' && docker cp '$STAGE/C0FWPOL.m' '$CONT:$REMOTE_P/C0FWPOL.m' && docker exec '$CONT' chown '$M_USER:$M_USER' '$REMOTE_P/C0FHIRLG.m' '$REMOTE_P/C0FHIRL.m' '$REMOTE_P/C0FWLAB.m' '$REMOTE_P/C0FWPOL.m'"
